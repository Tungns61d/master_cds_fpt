...
def detect_sign(frame, min_size_components = 240):
    original_image = frame.copy()
    binary_image = preprocess_image(frame)

    binary_image = removeSmallComponents(binary_image, min_size_components)

    binary_image = cv2.bitwise_and(binary_image,binary_image, mask=remove_other_color(frame))
    ...
    # coordinate, image, sign_type, text = localization(frame, 240,0.65, model, count, current_sign)
    if coordinate is not None:
        ...
        tl = [left, top]
        br = [right,bottom]
        # print(tl,br,'left,right')

        if (right> 160):
            direction = 'right'
            cv2.putText(frame,"Turn right",(left-15,bottom+20), font, 1,(0,0,255),2,cv2.LINE_4)
        else:
            cv2.putText(frame,"Turn left",(left,bottom+20), font, 1,(0,0,255),2,cv2.LINE_4)
            direction = 'left'
    else:
        direction = 'none'
    ...
    # # print(direction)
    cv2.imshow('Traffic Detect',frame)
    k = cv2.waitKey(1)
    if k == ord('q'): 
